	...
	if (m_pSprite != nullptr)
	{
		...
		if (m_pSprite->m_vList.size() > 0)
		{
			for (int iV = 0; iV < 6; iV++)
			{
				...
				obj->m_vList[iV].t = m_pSprite->m_vList[iV].t;
			}
		}
	}
}
...
void TActor::SetWorldMatrix(T_Math::FMatrix3x3& m)
{
	m_matWorld = m;
}
void TActor::SetViewTransform(T_Math::FMatrix3x3& matCamera)
{	
	TDxObject2D* obj = (TDxObject2D*)m_dxObj;
	for (int iv = 0; iv < obj->m_vListScreen.size(); iv++)
	{
		obj->m_vList[iv].p = obj->m_vList[iv].p * matCamera;
	}
}
TActor& TActor::SetWorldTransform()
{
	...
	for (int iv = 0; iv < obj->m_vListScreen.size(); iv++)
	{
		obj->m_vList[iv].p = obj->m_vListScreen[iv].p * m_matWorld;
	}
	return *this;
}
TActor& TActor::SetWorldTransform(T_Math::FMatrix3x3& m)
{
	TDxObject2D* obj = (TDxObject2D*)m_dxObj;
	for (int iv = 0; iv < obj->m_vListScreen.size(); iv++)
	{
		obj->m_vList[iv].p = obj->m_vListScreen[iv].p * m;
	}
	return *this;
}
...
TActor& TActor::Move(T_Math::FVector2 d)
{
	...
	TDxObject2D* obj = (TDxObject2D*)m_dxObj;
	m_rt.left	= obj->m_vList[0].p.X;
	m_rt.right	= obj->m_vList[1].p.X;
	m_rt.top	= obj->m_vList[0].p.Y;
	m_rt.bottom = obj->m_vList[2].p.Y;
	return *this;
}