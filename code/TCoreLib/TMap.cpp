#include "TMap.h"
void   TMap::LoadHeightMapTexture(T_STR texName)
{
	...
	pTexture2D->GetDesc(&m_HeightMapDesc);

	m_HeightList.resize(m_HeightMapDesc.Height * m_HeightMapDesc.Width);
		...
		D3D11_MAP_READ, 0, &mapped)))
	{
		...
		// RGBA
		for (int iRow = 0; iRow < m_HeightMapDesc.Height; iRow++)
		{
			// 한줄의 시작 인덱스
			UINT rowStart = iRow * mapped.RowPitch;
			for (int iCol = 0; iCol < m_HeightMapDesc.Width; iCol++)
			{
				// 한 픽셀 단위로 이동
				...
				m_HeightList[iRow * m_HeightMapDesc.Width + iCol] = (float)r;
			}
		}
