...
{
	m_fbxLoader.Init();
	TKgcFileFormat tModel1;
	// util , tool
	if (m_fbxLoader.Load("../../data/fbx/box.fbx", tModel1))
	{
		TKgcFileFormat::Export(&tModel1, L"box.kgc");
	}
	std::vector<TFbxModel*> model;
	TKgcFileFormat::Import(L"box.kgc",model);

	// engine , core
	//2)tModel1 = TObjectMgr::Load(L"box.kgc");
